...
struct ContentView: View {
    ...
    let uploadDescreptionUrl: URL = URL(string:
    "<YOUR_DEVTUNNEL_ENDPOINT>/api/File/uploaddesc")!
    let boundary: String = "Boundary-\(UUID().uuidString)"
    @State var supportFiles = [URL]()
                    ...
                    VStack {
                        ...
                        ZStack
                        {
                            ...
                            if issueDescription.isEmpty {
                                VStack {
                                    HStack {
                                        Text("Write the issue description here ...")
                        ...
                        Button(action: onSupportRequest, label: {
                            Text("Submit")
    ...
    private  func onSupportRequest() {
        ...
        URLSession.shared.dataTask(with: descreption) { data, resp, error in
            if let error = error {
                print(error)
                return
            }
    ...
    private func generateRequest(httpBody: Data, endpointUrl:URL) -> URLRequest {
